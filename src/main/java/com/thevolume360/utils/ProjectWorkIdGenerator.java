...
import com.thevolume360.domain.ProjectAddress;
import com.thevolume360.domain.enums.WorkType;

public class ProjectWorkIdGenerator {
	public static final int BASE_YEAR = 2015;
	private static final Random random = new Random();

	private static final String[] symbolPool = new String[] { "1", "2", "3",
	...
	public static String generate(ProjectAddress projectAddress, WorkType workType) {
		String districtPrefix = projectAddress.getDistrict().getShortCode()
				.toUpperCase();
		String workTypeShortCode = workType.getShortCode().toUpperCase();
		String seq = nextSequence(); // the sequence
		String main = districtPrefix + "-" + workTypeShortCode + "-" + seq;
	...
	private static String nextSequence() {
		...
		int random = ProjectWorkIdGenerator.random.nextInt(3); // Generate 3
																// digits at max
		if (s <= lastSecond) {
			if (s < lastSecond) {
				s = lastSecond;
